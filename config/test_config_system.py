...
def test_config_manager():
    ...
    try:
        ...
        if system_config:
            print(f"   ✅ Configuración del sistema: {system_config.system_name} v{system_config.version}")
        ...
        if performance_config:
            print(f"   ✅ Configuración de rendimiento: {performance_config.max_response_time}s")
        
        # Probar configuración de seguridad
        security_config = manager.get_security_config()
        if security_config:
            print(f"   ✅ Configuración de seguridad: JWT {'habilitado' if security_config.jwt_enabled else 'deshabilitado'}")
        ...
        if validation['valid']:
            ...
            print(f"   ⚠️  Validación de configuración con errores: {validation['errors']}")
...
def test_config_validator():
    ...
    try:
        ...
        if results:
            print(f"   ✅ Validación completada: {len(results)} configuraciones validadas")
        ...
        if summary:
            print(f"   ✅ Resumen de validación: {summary['valid_configs']}/{summary['total_configs']} válidas")
...
def test_dynamic_config_manager():
    ...
    try:
        ...
        if config_names:
            ...
            if config_hash:
                print(f"   ✅ Hash de configuración '{config_name}': {config_hash[:8]}...")
...
def test_config_module():
    ...
    try:
        ...
        if system_info:
            print(f"   ✅ Información del sistema: {system_info['name']} v{system_info['version']}")
        ...
        if performance_info:
            print(f"   ✅ Información de rendimiento: {performance_info['max_concurrent_operations']} operaciones")
...
def run_all_tests():
    ...
    if passed_tests == total_tests:
        print("🎉 ¡Todas las pruebas pasaron! El sistema de configuración está funcionando correctamente.")
        return True
    else:
