...
try:
    ...
    from .dynamic_config_manager import DynamicConfigManager, get_dynamic_config_manager, ConfigChangeEvent
...
def get_model_info() -> dict:
    ...
    return {
        ...
        'default_llm_model': config.get('default_llm_model', 'models/custom/shaili-personal-model')
    }
...
def get_performance_info() -> dict:
    ...
    return {
        ...
        'max_response_time': config.get('performance', {}).get('max_response_time', 3.0),
        'concurrent_queries': config.get('performance', {}).get('concurrent_queries', 20),
...
def initialize_config_module():
    ...
    try:
        ...
        if not system_config:
            print("⚠️  Advertencia: No se pudo cargar la configuración principal del sistema")
