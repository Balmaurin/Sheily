...
class ConfigManager:
    ...
    def _load_all_configs(self):
        ...
        try:
            ...
            self.sheily_token_config = self._load_json_config("sheily_token_config.json")
            self.sheily_token_metadata = self._load_json_config("sheily_token_metadata.json")
            self.advanced_training_config = self._load_json_config("advanced_training_config.json")
    ...
    def get_redis_config(self) -> Dict[str, Any]:
        ...
        return {
            'host': 'redis',
            'port': 6379,
            'password': 'shaili_redis_password'
        }
    ...
    def validate_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            required_fields = [
                'system_name', 'version', 'base_path', 'data_path',
                'models_path', 'default_embedding_model', 'default_llm_model'
            ]
            
            for field in required_fields:
                if field not in main_config:
                    validation_results['errors'].append(f"Campo requerido faltante: {field}")
            ...
            for component in required_components:
                if component not in components:
                    validation_results['errors'].append(f"Componente requerido faltante: {component}")
                    validation_results['valid'] = False
            ...
            for service in required_services:
                if service not in docker_config.get('services', {}):
                    validation_results['errors'].append(f"Servicio Docker requerido faltante: {service}")
                    validation_results['valid'] = False
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'security': {
                ...
                'two_factor_enabled': self.main_config['security']['two_factor_enabled'],
                'rate_limiting_enabled': self.main_config['security']['rate_limiting']['enabled']
            },
            'blockchain': {
