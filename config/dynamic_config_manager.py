...
class DynamicConfigManager:
    ...
    def _load_config_file(self, filename: str):
        ...
        try:
            ...
            config_name = filename.replace('.json', '').replace('.yml', '').replace('.yaml', '')
    ...
    def _detect_config_changes(self, config_name: str, old_hash: str, new_hash: str) -> Dict[str, Any]:
        ...
        try:
            with self.config_locks.get(config_name, threading.Lock()):
                ...
                if old_hash != config_hash:
                    changes = self._detect_config_changes(config_name, old_hash, config_hash)
                    event = ConfigChangeEvent(
                        config_name=config_name,
    ...
    def register_config_callback(self, config_name: str, callback: Callable[[ConfigChangeEvent], None]):
    ...
    def unregister_config_callback(self, config_name: str, callback: Callable[[ConfigChangeEvent], None]):
        ...
        if config_name in self.config_callbacks:
            try:
                ...
                logger.warning(f"Callback no encontrado para configuración: {config_name}")
    ...
    def _validate_module_init_config(self, config: Dict[str, Any], result: Dict[str, Any]):
        """Validaciones específicas para la configuración de inicialización de módulos"""
        for module_name, module_config in config.items():
            if 'dependencies' not in module_config:
                result['errors'].append(f"Dependencias faltantes en módulo: {module_name}")
                result['valid'] = False
    
    def _validate_rate_limits_config(self, config: Dict[str, Any], result: Dict[str, Any]):
