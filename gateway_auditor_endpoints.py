...
class GatewayEndpointAuditor:
    ...
    def detect_duplicates(self):
        ...
        for key, endpoints in groups.items():
            if len(endpoints) > 1:
                self.duplicates.append(
                    {"group": key, "endpoints": endpoints, "count": len(endpoints)}
                )
        ...
        specific_duplicates = [
            ...
            (
                [("GET", "/api/auth/tokens"), ("GET", "/api/auth/tokens/simple")],
                "Tokens de usuario",
            ),
            (
                [
        ...
        for endpoints, description in specific_duplicates:
            ...
            if len(existing) > 1:
                ...
                for method, path in existing:
                    status = (
                        "✅"
                        if self.endpoints_tested.get(f"{method} {path}", {}).get(
                            "working"
                        )
                        else "❌"
                    )
...
"""
    ...
    def generate_report(self):
        ...
        efficiency = (
            (self.metrics["working"] / self.metrics["total_found"]) * 100
            if self.metrics["total_found"] > 0
            else 0
        )
        ...
        cleanable = (
            ...
            + self.metrics["unused"]
            + self.metrics["problematic"]
        )
        if cleanable > 0:
            print(f"🧹 Endpoints que se pueden limpiar: {cleanable}")
            print(
                f"📉 Reducción potencial: {(cleanable/self.metrics['total_found'])*100:.1f}%"
            )
