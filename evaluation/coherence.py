...
class CoherenceEvaluator:
    def __init__(self, model_path: str = 'models/custom/shaili-personal-model'):
        ...
        self.logical_connectors = {
            ...
            'consecuencia': ['por tanto', 'así que', 'por consiguiente', 'por eso', 'entonces'],
            'contraste': ['pero', 'sin embargo', 'no obstante', 'aunque', 'a pesar de'],
            'adición': ['además', 'también', 'asimismo', 'igualmente', 'del mismo modo'],
            'ejemplo': ['por ejemplo', 'como', 'tal como', 'específicamente', 'en particular']
        }
        
        # Palabras clave comunes por dominio
        self.domain_keywords = {
            'ciencia': ['investigación', 'estudio', 'experimento', 'datos', 'análisis', 'resultados'],
            'tecnología': ['sistema', 'tecnología', 'aplicación', 'software', 'hardware', 'desarrollo'],
            'medicina': ['tratamiento', 'diagnóstico', 'síntomas', 'paciente', 'enfermedad', 'medicamento'],
            'educación': ['aprendizaje', 'enseñanza', 'estudiante', 'profesor', 'conocimiento', 'habilidad']
        }
    ...
    def evaluate_relevance(self, query: str, response: str) -> float:
        ...
        common_words = {'qué', 'cómo', 'cuál', 'dónde', 'cuándo', 'por', 'para', 'con', 'de', 'el', 'la', 'los', 'las', 'un', 'una', 'unos', 'unas', 'es', 'son', 'está', 'están', 'ser', 'estar', 'tener', 'hacer', 'ver', 'saber', 'poder', 'deber', 'querer'}
    ...
    def analyze_consistency(self, text: str) -> Dict[str, Any]:
        ...
        temporal_indicators = ['antes', 'después', 'durante', 'mientras', 'cuando', 'siempre', 'nunca', 'ahora', 'entonces', 'pronto', 'tarde']
    ...
    def get_detailed_coherence_analysis(self, query: str, response: str) -> Dict[str, Any]:
        """
...
def main():
    ...
    for i, example in enumerate(examples, 1):
        ...
        print(f"  Densidad de Conectores: {analysis['logical_structure']['connector_density']:.3f}")
        print(f"  Consistencia de Entidades: {analysis['consistency']['entity_consistency']:.3f}")
