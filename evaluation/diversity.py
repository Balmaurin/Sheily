...
class DiversityEvaluator:
    def __init__(self):
        ...
        try:
            ...
            self.stopwords = set(['el', 'la', 'de', 'que', 'y', 'a', 'en', 'un', 'es', 'se', 'no', 'te', 'lo', 'le', 'da', 'su', 'por', 'son', 'con', 'para', 'al', 'del', 'los', 'las', 'una', 'como', 'pero', 'sus', 'me', 'hasta', 'hay', 'donde', 'han', 'quien', 'están', 'estado', 'desde', 'todo', 'nos', 'durante', 'todos', 'uno', 'les', 'ni', 'contra', 'otros', 'ese', 'eso', 'ante', 'ellos', 'e', 'esto', 'mí', 'antes', 'algunos', 'qué', 'unos', 'yo', 'otro', 'otras', 'otra', 'él', 'tanto', 'esa', 'estos', 'mucho', 'quienes', 'nada', 'muchos', 'cual', 'poco', 'ella', 'estar', 'estas', 'algunas', 'algo', 'nosotros'])
    ...
    def lexical_diversity(self, text: str) -> Dict[str, float]:
        ...
        valid_tokens = [token for token in tokens if len(token) > 1 and token not in self.stopwords]
    ...
    def syntactic_complexity(self, text: str) -> Dict[str, Any]:
        ...
        patterns = {
            'conectores': ['pero', 'sin embargo', 'además', 'también', 'porque', 'ya que'],
    ...
    def evaluate_diversity(self, text: str) -> Dict[str, Any]:
        ...
        if not text or len(text.strip()) == 0:
            return {
                'diversity_score': 0.0,
                'lexical_metrics': {'type_token_ratio': 0.0, 'guiraud_index': 0.0, 'herdan_index': 0.0},
                'syntactic_metrics': {'avg_sentence_length': 0.0, 'max_syntax_depth': 0, 'pos_diversity': 0.0},
        ...
        semantic_score = min(semantic_metrics['bigram_entropy'] / 8.0, 1.0)  # Normalizar
