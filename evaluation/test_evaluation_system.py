...
def test_coherence_evaluator():
    ...
    try:
        ...
        for i, pair in enumerate(test_pairs, 1):
            coherence_score = evaluator.calculate_coherence(pair['query'], pair['response'])
            detailed_analysis = evaluator.get_detailed_coherence_analysis(pair['query'], pair['response'])
            
            results.append({
                'pair_id': i,
                'query': pair['query'],
                'response': pair['response'][:50] + "..." if len(pair['response']) > 50 else pair['response'],
...
def test_quality_pipeline():
    ...
    try:
        ...
        conversation = [
            ...
            {"role": "assistant", "content": "En la fotosíntesis, las plantas utilizan clorofila para capturar energía solar, que luego convierten en glucosa a través de complejas reacciones metabólicas."}
        ]
...
def test_package_imports():
    ...
    try:
        ...
        diversity_result = evaluation.evaluate_diversity("Texto de prueba para diversidad.")
        toxicity_result = evaluation.evaluate_toxicity("Texto de prueba para toxicidad.")
        coherence_result = evaluation.evaluate_coherence("¿Qué es la prueba?", "La prueba es un método de validación.")
