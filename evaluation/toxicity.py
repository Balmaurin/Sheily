...
class ToxicityEvaluator:
    ...
    def _load_toxic_lexicon(self, lexicon_path=None) -> Dict[str, Dict]:
        ...
        default_lexicon = {
            'insultos': {
                'palabras': ['idiota', 'estúpido', 'imbécil', 'tonto', 
                             'pendejo', 'gil', 'bobo', 'retrasado', 'bruto'],
                'severidad': 0.7
            },
            'discriminacion': {
                'palabras': ['negro', 'indio', 'sudaca', 'extranjero', 
                             'inmigrante', 'discriminación', 'racista'],
                'severidad': 0.8
            },
            'sexismo': {
                'palabras': ['machista', 'feminazi', 'sexista', 
                             'débil', 'histérica', 'feminista'],
                'severidad': 0.6
            },
            'violencia': {
                'palabras': ['matar', 'golpear', 'agredir', 'violencia', 
                             'muerte', 'destruir', 'asesinar', 'pegar'],
                'severidad': 0.9
            },
            'odio': {
                ...
                with open(lexicon_path, 'r', encoding='utf-8') as f:
                    ...
                    for category, data in custom_lexicon.items():
                        if category in default_lexicon:
                            default_lexicon[category]['palabras'].extend(data.get('palabras', []))
                ...
                if matches:
                    ...
                    for match in matches:
                        ...
                        if match_start != -1:
                            ...
                            has_negation = any(neg_word in context for neg_word in self.negation_words)
    ...
    def calculate_toxicity_score(self, text: str) -> Dict[str, Union[float, Dict]]:
        ...
        if toxic_language:
            for category, data in toxic_language.items():
                base_toxicity += data['severidad'] * min(data['count'], 3)  # Limitar impacto por categoría
    ...
    def evaluate_toxicity(self, 
                           text: str, 
                           threshold: float = 0.3) -> Dict[str, Union[bool, float, Dict]]:
        """
...
def main():
    ...
    for i, text in enumerate(texts, 1):
        ...
        if toxicity_result['toxic_categories']:
            print(f"Categorías tóxicas: {', '.join(toxicity_result['toxic_categories'])}")
