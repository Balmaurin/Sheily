...
class MemorySummarizer:
    ...
    def generate_summary(self, messages: List[MemoryMessage]) -> str:
        ...
        assistant_messages = [msg for msg in important_messages if msg.role == "assistant"]
...
class ShortTermMemoryManager:
    ...
    def _init_database(self):
        ...
        try:
            ...
            log_error("❌ Error inicializando base de datos de memoria a corto plazo", e)
    ...
    def add_message(self, session_id: str, role: str, content: str, 
        ...
        try:
            with self.lock:
                ...
                self.logger.debug(f"Mensaje añadido: {message_id} a sesión {session_id}")
                return message_id
        except Exception as e:
    ...
    def _manage_session_limits(self, session_id: str):
        ...
        try:
            ...
            if len(messages) > self.config.max_messages:
                ...
                messages_to_remove = messages[:len(messages) - self.config.max_messages]
            ...
            if self.config.auto_summarize and session.total_tokens > self.config.summary_threshold:
        ...
        try:
            with self.lock:
                ...
                return {
                    ...
                    "avg_importance": sum(msg.importance_score for msg in messages) / len(messages) if messages else 0
                }
        except Exception as e:
    ...
    def backup_memory(self, backup_path: str = None):
        ...
        try:
            ...
            backup_path = backup_path or f"short_term/backup/memory_backup_{int(time.time())}.json"
            
            backup_data = {
                "sessions": {sid: asdict(session) for sid, session in self.sessions.items()},
                "messages": {sid: [asdict(msg) for msg in messages] 
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    session.last_accessed, session.message_count, session.total_tokens,
                    session.summary, json.dumps(session.metadata) if session.metadata else None
                ))
                conn.commit()
        except Exception as e:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    message.tokens, message.timestamp, json.dumps(message.embedding) if message.embedding else None,
                    json.dumps(message.metadata) if message.metadata else None,
                    message.importance_score, message.access_count, message.last_accessed
                ))
                conn.commit()
        except Exception as e:
    ...
    def _cleanup_old_sessions(self):
        ...
        try:
            ...
            if sessions_to_delete:
                self.logger.info(f"Limpieza automática: {len(sessions_to_delete)} sesiones eliminadas")
        except Exception as e:
...
def main():
    ...
    manager.add_message(session_id, "assistant", "La inteligencia artificial es un campo de la informática que busca crear sistemas capaces de realizar tareas que normalmente requieren inteligencia humana.")
    manager.add_message(session_id, "user", "¿Cuáles son los tipos principales de IA?")
    manager.add_message(session_id, "assistant", "Los tipos principales incluyen: 1) IA débil (especializada), 2) IA fuerte (general), 3) Machine Learning, 4) Deep Learning.")
