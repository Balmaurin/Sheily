...
class DependenciesManager:
    ...
    def check_system_dependency(self, package_name: str) -> DependencyInfo:
        ...
        try:
            ...
            # Verificar si está instalada
            try:
                result = subprocess.run([package_name, '--version'], 
            ...
            logger.error(f"Error verificando dependencia del sistema {package_name}: {e}")
            return DependencyInfo(
                name=package_name,
                version='unknown',
                type='system',
    ...
    def install_python_dependency(self, package_name: str, version: str = None) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(f"Error instalando dependencia Python {package_name}: {result.stderr}")
                return False
    ...
    def install_node_dependency(self, package_name: str, version: str = None) -> bool:
        ...
        try:
            ...
            result = subprocess.run(['npm', 'install', package_spec],
                                  capture_output=True, text=True, cwd=self.deps_dir)
            ...
            if result.returncode == 0:
                ...
                logger.error(f"Error instalando dependencia Node.js {package_name}: {result.stderr}")
                return False
    ...
    def get_dependency_stats(self) -> DependencyStats:
        ...
        try:
            ...
            missing_deps = sum(1 for dep in dependencies if not dep.installed and dep.required)
...
if __name__ == "__main__":
    ...
    print(f"Estadísticas: {stats.installed_dependencies}/{stats.total_dependencies} instaladas")
