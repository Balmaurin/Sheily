...
class DependencyValidator:
        ...
        self.performance_benchmarks = {
            'python': {
                'torch': {
                    ...
                    'performance_improvements': ['Mejor rendimiento en GPU', 'Optimizaciones de memoria']
                },
                'transformers': {
                    'min_version': '4.20.0',
                    'recommended_version': '4.25.0',
                    'performance_improvements': ['Inferencia más rápida', 'Menor uso de memoria']
                }
            }
        }
    
    def validate_python_dependency(self, package_name: str, required_version: str) -> DependencyValidationResult:
        ...
        try:
            ...
            if is_installed and installed_version:
                ...
                if not self._check_version_compatibility(installed_version, required_version):
                    is_compatible = False
                    compatibility_issues.append(f"Versión instalada {installed_version} no cumple con requerimiento {required_version}")
                    validation_score -= 0.3
                
                # Verificar vulnerabilidades de seguridad
                security_issues.extend(self._check_security_vulnerabilities('python', package_name, installed_version))
                if security_issues:
                    validation_score -= 0.4
                
                # Verificar problemas de rendimiento
                performance_issues.extend(self._check_performance_issues('python', package_name, installed_version))
                if performance_issues:
                    validation_score -= 0.2
                
                # Generar recomendaciones
                recommendations.extend(self._generate_recommendations('python', package_name, installed_version))
                
                # Verificar compatibilidad con otras dependencias
                if package_name in self.compatibility_matrix.get('python', {}):
                    dep_compatibility = self.compatibility_matrix['python'][package_name]
                        ...
                        if dep_name != 'incompatible':
                            try:
                                if dep_name == 'python':
                                    ...
                                    current_version = pkg_resources.get_distribution(dep_name).version
                                
                                if not self._check_version_compatibility(current_version, dep_version):
                                    compatibility_issues.append(f"Incompatible con {dep_name} {current_version}, requiere {dep_version}")
                                    validation_score -= 0.2
                            except:
                                compatibility_issues.append(f"No se pudo verificar compatibilidad con {dep_name}")
    ...
    def validate_node_dependency(self, package_name: str, required_version: str) -> DependencyValidationResult:
        """Valida una dependencia de Node.js"""
        try:
            # Verificar si está instalada
            try:
                result = subprocess.run(['npm', 'list', package_name], 
                                      capture_output=True, text=True, cwd=self.deps_dir)
            ...
            if is_installed and installed_version:
                ...
                # Verificar vulnerabilidades de seguridad
                security_issues.extend(self._check_security_vulnerabilities('node', package_name, installed_version))
                if security_issues:
                    validation_score -= 0.4
                
                # Verificar problemas de rendimiento
                performance_issues.extend(self._check_performance_issues('node', package_name, installed_version))
                if performance_issues:
                    validation_score -= 0.2
                
                # Generar recomendaciones
                recommendations.extend(self._generate_recommendations('node', package_name, installed_version))
                        ...
                        try:
                            ...
                            if dep_name == 'node':
                                ...
                                if result.returncode == 0:
                                    ...
                                    if not self._check_version_compatibility(current_version, dep_version):
                                        compatibility_issues.append(f"Incompatible con Node.js {current_version}, requiere {dep_version}")
                                        validation_score -= 0.2
                            elif dep_name == 'npm':
                                ...
                                if result.returncode == 0:
                                    ...
                                    if not self._check_version_compatibility(current_version, dep_version):
                                        compatibility_issues.append(f"Incompatible con npm {current_version}, requiere {dep_version}")
                                        validation_score -= 0.2
                        except:
                            compatibility_issues.append(f"No se pudo verificar compatibilidad con {dep_name}")
                            validation_score -= 0.1
    ...
    def validate_system_dependency(self, package_name: str, required_version: str) -> DependencyValidationResult:
        ...
        try:
            ...
            if is_installed and installed_version:
                ...
                # Generar recomendaciones
                recommendations.extend(self._generate_recommendations('system', package_name, installed_version))
            else:
                validation_score = 0.0
    ...
    def validate_all_dependencies(self, dependencies_config: Dict[str, Any]) -> List[DependencyValidationResult]:
        ...
        for package_name, config in dependencies_config.get('python_dependencies', {}).items():
            result = self.validate_python_dependency(package_name, config.get('version', 'unknown'))
            results.append(result)
        
        # Validar dependencias de Node.js
        logger.info("Validando dependencias de Node.js...")
        for package_name, config in dependencies_config.get('node_dependencies', {}).items():
            result = self.validate_node_dependency(package_name, config.get('version', 'unknown'))
            results.append(result)
        
        # Validar dependencias del sistema
        logger.info("Validando dependencias del sistema...")
        for package_name, config in dependencies_config.get('system_dependencies', {}).items():
            result = self.validate_system_dependency(package_name, config.get('version', 'unknown'))
            results.append(result)
    ...
    def _check_version_compatibility(self, installed_version: str, required_version: str) -> bool:
        ...
        try:
            ...
            required_clean = required_version.replace('>=', '').replace('<=', '').replace('==', '').replace('~', '').replace('^', '')
            ...
            if '>=' in required_version:
                ...
                # Caret requirement (^)
                return installed_ver >= required_ver and installed_ver < version.parse(f"{required_ver.major + 1}")
            else:
                # Sin operador, asumir >=
                return installed_ver >= required_ver
    ...
    def _check_security_vulnerabilities(self, package_type: str, package_name: str, installed_version: str) -> List[str]:
        """Verifica vulnerabilidades de seguridad conocidas"""
        vulnerabilities = []
        
        try:
            if package_type in self.security_database and package_name in self.security_database[package_type]:
                security_info = self.security_database[package_type][package_name]
                
                for vuln_version in security_info['versions']:
                    if self._check_version_compatibility(installed_version, vuln_version):
                        vulnerabilities.append(f"{security_info['severity'].upper()}: {security_info['description']} ({security_info['cve']})")
    ...
    def _check_performance_issues(self, package_type: str, package_name: str, installed_version: str) -> List[str]:
        """Verifica problemas de rendimiento conocidos"""
        issues = []
        
        try:
            if package_type in self.performance_benchmarks and package_name in self.performance_benchmarks[package_type]:
                perf_info = self.performance_benchmarks[package_type][package_name]
                
                if not self._check_version_compatibility(installed_version, perf_info['min_version']):
                    issues.append(f"Versión {installed_version} puede tener problemas de rendimiento. Recomendado: {perf_info['recommended_version']}")
    ...
    def _generate_recommendations(self, package_type: str, package_name: str, installed_version: str) -> List[str]:
            ...
            if package_type == 'python':
                if package_name == 'torch' and installed_version:
                    if version.parse(installed_version) < version.parse('1.13.0'):
                        recommendations.append("Actualizar PyTorch a la versión 1.13.0+ para mejor rendimiento")
                
                elif package_name == 'transformers' and installed_version:
                    if version.parse(installed_version) < version.parse('4.25.0'):
                        recommendations.append("Actualizar Transformers a la versión 4.25.0+ para mejor rendimiento")
                
                elif package_name == 'numpy' and installed_version:
                    if version.parse(installed_version) < version.parse('1.24.0'):
                        recommendations.append("Actualizar NumPy a la versión 1.24.0+ para mejor compatibilidad")
            
            elif package_type == 'node':
                if package_name == 'react' and installed_version:
                    if version.parse(installed_version) < version.parse('18.2.0'):
                        recommendations.append("Actualizar React a la versión 18.2.0+ para mejor rendimiento")
                
                elif package_name == 'next' and installed_version:
                    if version.parse(installed_version) < version.parse('13.0.0'):
                        recommendations.append("Actualizar Next.js a la versión 13.0.0+ para nuevas características")
            
            # Recomendaciones generales
            if package_type in self.performance_benchmarks and package_name in self.performance_benchmarks[package_type]:
                perf_info = self.performance_benchmarks[package_type][package_name]
                if 'recommended_version' in perf_info:
                    recommendations.append(f"Considerar actualizar a {perf_info['recommended_version']} para mejor rendimiento")
    ...
    def get_validation_summary(self) -> ValidationSummary:
        ...
        outdated_deps = len([r for r in self.validation_results if r.is_installed and not r.is_compatible])
        ...
        overall_score = sum(r.validation_score for r in self.validation_results) / total_deps
    ...
    def save_validation_report(self, filepath: str = None) -> str:
        ...
        if filepath is None:
            ...
            filepath = self.deps_dir / "validation_reports" / f"validation_report_{timestamp}.json"
    ...
    def print_validation_report(self):
        ...
        for result in self.validation_results:
            ...
            print(f"      Versión instalada: {result.installed_version or 'No instalada'}")
