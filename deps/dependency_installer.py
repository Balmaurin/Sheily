...
class DependencyInstaller:
    ...
    def _update_progress(self, current: int, total: int, current_package: str, current_type: str):
        """Actualiza el progreso de instalación"""
        if self.progress_callback:
            progress = InstallationProgress(
                total_packages=total,
                installed_packages=current,
                failed_packages=len([r for r in self.installation_log if not r['success']]),
        ...
        try:
            ...
            install_commands = {
                'ubuntu': {
                    ...
                    'docker-compose': ['sudo', 'apt-get', 'update', '&&', 'sudo', 'apt-get', 'install', '-y', 'docker-compose'],
                ...
                'centos': {
                    ...
                    'node': ['curl', '-fsSL', 'https://rpm.nodesource.com/setup_18.x', '|', 'sudo', 'bash', '-', '&&', 'sudo', 'yum', 'install', '-y', 'nodejs'],
                    'npm': ['sudo', 'yum', 'install', '-y', 'npm'],
                    'git': ['sudo', 'yum', 'install', '-y', 'git'],
                    'docker': ['sudo', 'yum', 'install', '-y', 'docker'],
                    'docker-compose': ['sudo', 'curl', '-L', '"https://github.com/docker/compose/releases/download/v2.0.0/docker-compose-$(uname -s)-$(uname -m)"', '-o', '/usr/local/bin/docker-compose', '&&', 'sudo', 'chmod', '+x', '/usr/local/bin/docker-compose'],
                    'postgresql': ['sudo', 'yum', 'install', '-y', 'postgresql-server', 'postgresql-contrib'],
            ...
            result = subprocess.run(command, capture_output=True, text=True, timeout=self.install_config['timeout'])
            ...
            if result.returncode == 0:
                ...
                logger.info(f"Dependencia del sistema instalada: {package_name} {version}")
            ...
            logger.error(f"Error instalando dependencia del sistema {package_name}: {e}")
            return installation_result
    
    def install_python_dependency(self, package_name: str, version: str = None) -> InstallationResult:
        ...
        try:
            ...
            for attempt in range(self.install_config['retry_attempts']):
                try:
                    ...
                    if result.returncode == 0:
                        # Verificar instalación
                        installed_version = self._get_python_package_version(package_name)
                        ...
                        installation_result = InstallationResult(
                            ...
                            error_message=None,
                            installation_time=time.time() - start_time,
                            dependencies_installed=self._extract_installed_dependencies(result.stdout)
                        )
                        
                        logger.info(f"Dependencia Python instalada: {package_name} {installed_version}")
                        return installation_result
                    else:
                        if attempt < self.install_config['retry_attempts'] - 1:
                            logger.warning(f"Intento {attempt + 1} fallido para {package_name}, reintentando...")
                            time.sleep(2)
                            continue
                        else:
                            raise Exception(f"Error en instalación: {result.stderr}")
                    ...
                    if attempt < self.install_config['retry_attempts'] - 1:
                        logger.warning(f"Timeout en intento {attempt + 1} para {package_name}, reintentando...")
                        continue
                    else:
    ...
    def install_node_dependency(self, package_name: str, version: str = None) -> InstallationResult:
        ...
        try:
            ...
            for attempt in range(self.install_config['retry_attempts']):
                try:
                    result = subprocess.run(command, capture_output=True, text=True, 
                                          cwd=self.deps_dir, timeout=self.install_config['timeout'])
                    ...
                    if result.returncode == 0:
                        ...
                        logger.info(f"Dependencia Node.js instalada: {package_name} {installed_version}")
                        return installation_result
                    else:
                        if attempt < self.install_config['retry_attempts'] - 1:
                            logger.warning(f"Intento {attempt + 1} fallido para {package_name}, reintentando...")
    ...
    def install_all_dependencies(self, dependencies_config: Dict[str, Any]) -> List[InstallationResult]:
        ...
        try:
            ...
            for dep_type in self.install_config['install_order']:
                if dep_type == 'system':
                    total_packages += len(dependencies_config.get('system_dependencies', {}))
                elif dep_type == 'python':
                    total_packages += len(dependencies_config.get('python_dependencies', {}))
                elif dep_type == 'node':
                    total_packages += len(dependencies_config.get('node_dependencies', {}))
            ...
            for dep_type in self.install_config['install_order']:
                ...
                if dep_type == 'system':
                    results.extend(self._install_system_dependencies(dependencies_config.get('system_dependencies', {}), 
                                                                   total_packages, installed_count))
                elif dep_type == 'python':
                    results.extend(self._install_python_dependencies(dependencies_config.get('python_dependencies', {}), 
                                                                   total_packages, installed_count))
                elif dep_type == 'node':
                    results.extend(self._install_node_dependencies(dependencies_config.get('node_dependencies', {}), 
                                                                 total_packages, installed_count))
                
                installed_count = len([r for r in results if r.success])
            
            logger.info(f"Instalación completada. {installed_count}/{total_packages} paquetes instalados exitosamente")
            return results
    ...
    def _install_system_dependencies(self, system_deps: Dict[str, Any], total: int, current: int) -> List[InstallationResult]:
        ...
        for package_name, config in system_deps.items():
            ...
            if config.get('required', False) or not self.install_config['skip_optional']:
                self._update_progress(current + len(results), total, package_name, 'system')
    ...
    def _install_python_dependencies(self, python_deps: Dict[str, Any], total: int, current: int) -> List[InstallationResult]:
        """Instala dependencias de Python"""
        results = []
        
        # Instalar en paralelo usando ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=self.install_config['max_workers']) as executor:
            futures = []
            ...
            for package_name, config in python_deps.items():
                ...
                if config.get('required', False) or not self.install_config['skip_optional']:
                    future = executor.submit(self.install_python_dependency, package_name, config.get('version'))
    ...
    def _install_node_dependencies(self, node_deps: Dict[str, Any], total: int, current: int) -> List[InstallationResult]:
        ...
        with ThreadPoolExecutor(max_workers=self.install_config['max_workers']) as executor:
            ...
            for package_name, config in node_deps.items():
                ...
                if config.get('required', False) or not self.install_config['skip_optional']:
                    future = executor.submit(self.install_node_dependency, package_name, config.get('version'))
                    futures.append((package_name, future))
    ...
    def _get_node_package_version(self, package_name: str) -> Optional[str]:
        ...
        try:
            result = subprocess.run(['npm', 'list', package_name], capture_output=True, text=True, cwd=self.deps_dir)
            if result.returncode == 0:
    ...
    def get_installation_summary(self) -> Dict[str, Any]:
        ...
        successful_installations = len([r for r in self.installation_log if r['success']])
        failed_installations = total_installations - successful_installations
        
        # Estadísticas por tipo
        python_installations = [r for r in self.installation_log if r['package_type'] == 'python']
        node_installations = [r for r in self.installation_log if r['package_type'] == 'node']
        system_installations = [r for r in self.installation_log if r['package_type'] == 'system']
        
        # Tiempo total de instalación
        total_time = sum(r['installation_time'] for r in self.installation_log)
        
        # Paquetes fallidos
        failed_packages = [r['package_name'] for r in self.installation_log if not r['success']]
    ...
    def save_installation_report(self, filepath: str = None) -> str:
        ...
        if filepath is None:
            ...
            filepath = self.deps_dir / "installation_reports" / f"installation_report_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    print(f"Instalación completada: {summary['successful_installations']}/{summary['total_installations']} exitosas")
