        ...
        return {
            ...
            "memory": {
                ...
                "status": (
                    ...
                    else "🟡" if memory.percent < 90 else "🔴"
                ),
            },
            "disk": {
                ...
                "status": (
                    "🟢" if disk.percent < 70 else "🟡" if disk.percent < 90 else "🔴"
                ),
            },
        }
    except Exception as e:
...
def print_dashboard():
    ...
    for service_name, info in services_status.items():
        print(
            f"{info['status']} {service_name:<20} {info['health']:<12} Puerto: {info['port']:<6} Tiempo: {info['response_time']}"
        )
        if "error" in info:
    ...
    if "error" not in resources:
        print(
            f"{resources['cpu']['status']} CPU:    {resources['cpu']['percent']:>5.1f}%"
        )
        print(
            f"{resources['memory']['status']} Memoria: {resources['memory']['percent']:>5.1f}% ({resources['memory']['used']} / {resources['memory']['total']})"
        )
        print(
            f"{resources['disk']['status']} Disco:   {resources['disk']['percent']:>5.1f}% ({resources['disk']['used']} / {resources['disk']['total']})"
        )
    else:
    ...
    running_services = [
        name for name, info in services_status.items() if "RUNNING" in info["status"]
    ]
