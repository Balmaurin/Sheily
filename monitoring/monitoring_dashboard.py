...
class MonitoringDashboard:
    ...
    def create_model_metrics_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Inference Time (ms)', 'GPU Usage (%)', 'Requests/min', 'Error Rate (%)'),
    ...
    def create_branch_metrics_chart(self, df: pd.DataFrame) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Training Progress (%)', 'Accuracy Score (%)', 'Loss Value', 'Active Adapters'),
            specs=[[{"secondary_y": False}, {"secondary_y": False}],
                   [{"secondary_y": False}, {"secondary_y": False}]]
        )
        ...
        fig = go.Figure(data=[go.Table(
            ...
            cells=dict(
                values=[
                    ...
                    ['Resolved' if x else 'Active' for x in recent_alerts['resolved']]
                ],
                fill_color='lavender',
                align='left',
                font=dict(size=10)
...
app.layout = html.Div([
    # Header
    html.Div([
        html.H1("🚀 Shaili AI Monitoring Dashboard", 
                style={'textAlign': 'center', 'color': '#1f77b4', 'marginBottom': 20}),
        html.Div([
    ...
    html.Div([
        html.H2("📊 System Metrics", style={'textAlign': 'center', 'color': '#1f77b4'}),
        dcc.Graph(id='system-metrics-chart')
    ], style={'marginBottom': 40}),
    
    # Métricas del modelo
    html.Div([
        html.H2("🤖 Model Metrics", style={'textAlign': 'center', 'color': '#1f77b4'}),
        dcc.Graph(id='model-metrics-chart')
    ], style={'marginBottom': 40}),
    
    # Métricas de ramas
    html.Div([
        html.H2("🌿 Branch Metrics", style={'textAlign': 'center', 'color': '#1f77b4'}),
        dcc.Graph(id='branch-metrics-chart')
    ], style={'marginBottom': 40}),
    
    # Alertas
    html.Div([
        html.H2("🚨 Recent Alerts", style={'textAlign': 'center', 'color': '#1f77b4'}),
    ...
    dcc.Interval(
        ...
        interval=dashboard.update_interval,
        n_intervals=0
    ),
    
    # Sección de chat (nueva)
    html.Div([
        html.H2("💬 Chat with LLM", style={'textAlign': 'center', 'color': '#1f77b4'}),
        html.Div(id='chat-history', style={'height': '300px', 'overflowY': 'scroll', 'border': '1px solid #ddd', 'padding': '10px', 'marginBottom': '10px'}),
        dcc.Input(
            id='user-chat-input',
            type='text',
            placeholder='Escribe tu mensaje aquí...',
            style={'width': 'calc(100% - 20px)', 'padding': '10px', 'border': '1px solid #ccc'}
        ),
        html.Button('Enviar', id='send-chat-btn', n_clicks=0, 
                   style={'marginLeft': '10px', 'padding': '10px 15px', 'backgroundColor': '#1f77b4', 'color': 'white', 'border': 'none'})
    ], style={'marginBottom': 40}),
...
@app.callback(
    ...
    [Input('send-chat-btn', 'n_clicks'),
     Input('user-chat-input', 'n_submit')], # Para enviar con Enter
    [dash.State('user-chat-input', 'value'),
     dash.State('chat-storage', 'data')]
)
def handle_chat_input(send_btn_clicks, n_submit, user_message, chat_history):
    ...
    if (button_id == 'send-chat-btn' and send_btn_clicks > 0) or (button_id == 'user-chat-input' and n_submit):
        ...
        llm_messages = [{'role': msg['role'], 'content': msg['content']} for msg in chat_history]

        try:
            # Enviar mensaje al servidor LLM
            response = requests.post("http://127.0.0.1:5000/chat", json={"messages": llm_messages}, timeout=120)
        ...
        for msg in chat_history:
            style = {'textAlign': 'right', 'color': '#1f77b4', 'marginBottom': '5px'} if msg['role'] == 'user' else {'textAlign': 'left', 'color': '#333', 'marginBottom': '5px'}
