...
def compare_table_data(duckdb_path: str, table_name: str, postgres_url: str, model_class):
    """
    ...
    try:
        ...
        duckdb_columns = [desc[0] for desc in duckdb_conn.execute(duckdb_query).description]
        ...
        if len(duckdb_data) != len(pg_data):
            logger.error(f"Número de registros no coincide: DuckDB {len(duckdb_data)}, PostgreSQL {len(pg_data)}")
            return False
        ...
        for duckdb_row in duckdb_data:
            ...
            matching_pg_record = next(
                (pg_rec for pg_rec in pg_data_dicts 
                 if all(
            ...
            if not matching_pg_record:
                logger.error(f"No se encontró registro equivalente en PostgreSQL: {duckdb_dict}")
                return False
