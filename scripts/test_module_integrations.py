...
class IntegrationTester:
        ...
        for module_name, class_name in critical_modules:
            try:
                ...
                if hasattr(module, class_name):
                    self.test_result(f"Import {module_name}", True, f"Clase {class_name} disponible")
                else:
                    self.test_result(f"Import {module_name}", False, f"Clase {class_name} no encontrada")
            except ImportError as e:
                self.test_result(f"Import {module_name}", False, f"Error de importación: {e}")
            except Exception as e:
                self.test_result(f"Import {module_name}", False, f"Error inesperado: {e}")
        ...
        try:
            ...
            if db_path.exists():
                try:
                    ...
                    conn.close()
                    self.test_result("Knowledge Base DB", True, f"{count} registros encontrados")
                except Exception as e:
                    self.test_result("Knowledge Base DB", False, f"Error consultando: {e}")
            else:
            ...
            if db_path.exists():
                try:
                    ...
                    count = cursor.fetchone()[0]
                    conn.close()
                    self.test_result("Embeddings DB", True, f"{count} embeddings encontrados")
                except Exception as e:
            ...
            if db_path.exists():
                try:
                    ...
                    count = cursor.fetchone()[0]
                    conn.close()
                    self.test_result("Backend DB", True, f"{count} usuarios encontrados")
                except Exception as e:
        ...
        for config_file in config_files:
            ...
            if config_path.exists():
                try:
                    ...
                    self.test_result(f"Config {config_file}", True, f"{len(config_data)} configuraciones cargadas")
                except json.JSONDecodeError as e:
                    self.test_result(f"Config {config_file}", False, f"JSON inválido: {e}")
                except Exception as e:
                    self.test_result(f"Config {config_file}", False, f"Error leyendo: {e}")
            else:
                self.test_result(f"Config {config_file}", False, "Archivo no encontrado")
    ...
    def test_llm_system(self):
        ...
        if model_path.exists():
            ...
            if model_files:
                self.test_result("LLM Models", True, f"{len(model_files)} modelos GGUF encontrados")
            else:
    ...
    def test_frontend_backend_connection(self):
        ...
        for file_path in frontend_files:
            ...
            if full_path.exists():
                ...
                self.test_result(f"Frontend {file_path}", False, "Archivo no encontrado")
    ...
    def test_module_communication(self):
        ...
        try:
            ...
            if hasattr(initializer, 'initialize_core_modules'):
                self.test_result("Module Communication", True, "ModuleInitializer responde")
            else:
                self.test_warning("Module Communication", "ModuleInitializer sin método initialize_core_modules")
                
        except ImportError as e:
            self.test_result("Module Communication", False, f"Error importando ModuleInitializer: {e}")
        except Exception as e:
            self.test_result("Module Communication", False, f"Error en comunicación: {e}")
        ...
        try:
            ...
            if hasattr(manager, 'process_query'):
                self.test_result("Integration Manager", True, "IntegrationManager responde")
            else:
                self.test_warning("Integration Manager", "IntegrationManager sin método process_query")
    ...
    def test_blockchain_integration(self):
        ...
        for file_path in blockchain_files:
            ...
            if full_path.exists():
                ...
                self.test_result(f"Blockchain {file_path}", False, "Archivo no encontrado")
