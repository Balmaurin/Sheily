...
from modules.core.enhanced_daily_exercise_generator import EnhancedDailyExerciseGenerator
...
class EnhancedDailyExerciseScheduler:
    ...
    async def generate_exercises(self):
        ...
        try:
            ...
            exercises_by_branch = await self.generator.generate_enhanced_daily_exercises()
            
            total_exercises = sum(len(exercises) for exercises in exercises_by_branch.values())
            total_questions = sum(
            ...
            for branch, exercises in exercises_by_branch.items():
                branch_questions = sum(len(exercise.questions) for exercise in exercises)
                logger.info(f"   - {branch}: {len(exercises)} ejercicios, {branch_questions} preguntas")
        ...
        try:
            ...
            for branch, exercises in exercises_by_branch.items():
                ...
                for exercise in exercises:
                    ...
                    for i, question in enumerate(exercise.questions[:3]):  # Solo las primeras 3
                        logger.info(f"       Q{i+1}: {question['question'][:50]}...")
                    
                    if len(exercise.questions) > 3:
                        logger.info(f"       ... y {len(exercise.questions) - 3} preguntas más")
                    logger.info("")
    ...
    async def show_statistics(self):
        ...
        try:
            ...
            logger.info(f"Promedio de preguntas por ejercicio: {total_questions/total_exercises:.1f}")
            ...
            for difficulty, stats in difficulty_stats.items():
                logger.info(f"  {difficulty}: {stats['exercises']} ejercicios, {stats['questions']} preguntas")
...
async def main():
    """Función principal"""
    parser = argparse.ArgumentParser(description="Generador mejorado de ejercicios diarios")
    parser.add_argument(
        "command",
