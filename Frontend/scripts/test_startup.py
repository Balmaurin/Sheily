...
class StartupTester:
    ...
    def test_package_json(self) -> bool:
        ...
        try:
            ...
            if package_data.get("name") == "sheily-landing-next":
                ...
                self.log_test("Nombre del proyecto", False, f"Esperado: sheily-landing-next, Encontrado: {package_data.get('name')}")
                return False
    ...
    def test_system_dependencies(self) -> bool:
        ...
        try:
            ...
            if major_version >= 18:
                self.log_test("Versión de Node.js", True, f"Versión {major_version} >= 18")
            else:
                self.log_test("Versión de Node.js", False, f"Versión {major_version} < 18")
                all_passed = False
    ...
    def test_environment_setup(self) -> bool:
        ...
        if env_path.exists():
            ...
            self.log_test("Archivo .env.local", False, "No encontrado (se creará automáticamente)")
        ...
        if sounds_dir.exists():
            ...
            self.log_test("Directorio de sonidos", False, "No encontrado (se creará automáticamente)")
            # No es crítico, solo informativo
