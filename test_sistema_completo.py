...
def test_llm_server():
    ...
    try:
        ...
        payload = {"prompt": "Hola, ¿cómo estás?", "max_tokens": 50, "temperature": 0.7}

        response = requests.post(
            "http://localhost:8005/generate", json=payload, timeout=30
        )
...
def test_auth_system():
    ...
    try:
        ...
        login_payload = {"username": "test_user", "password": "invalid_password"}

        response = requests.post(
            "http://localhost:8000/api/auth/login", json=login_payload, timeout=5
        )
...
def test_integration_modules():
    ...
    try:
        ...
        if isinstance(result, dict) and "success" in result:
            ...
            print(
                f"   🎯 Dominio detectado: {result.get('routing', {}).get('domain', 'N/A')}"
            )
            return True
        else:
...
def main():
    ...
    if passed == total:
        ...
        print(
            "✅ Sistema mayormente funcional con algunos componentes opcionales fallando"
        )
    else:
