...
def cleanup_backend_endpoints():
    ...
    try:
        ...
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: GET /api/auth/tokens - Usar /api/auth/tokens/simple que no requiere auth", content, flags=re.DOTALL)
            changes_made += 1
        ...
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: GET /api/chat/qwen/history/:session_id - Contiene '/qwen'", content, flags=re.DOTALL)
            changes_made += 1
        ...
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: POST /api/chat/qwen/history/:session_id/clear - Contiene '/qwen'", content, flags=re.DOTALL)
            changes_made += 1
        ...
        pattern = r"app\.post\s*\(\s*['\"]/api/chat/qwen/history/:session_id/clear['\"]][^}]*}\s*\);?"
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: POST /api/chat/qwen/history/:session_id/clear - Contiene '/clear'", content, flags=re.DOTALL)
            changes_made += 1
            print(f"✅ Eliminado: POST /api/chat/qwen/history/:session_id/clear")
        ...
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: POST /api/chat/qwen - Contiene '/qwen'", content, flags=re.DOTALL)
            changes_made += 1
        ...
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: POST /api/chat/qwen/reload - Contiene '/qwen'", content, flags=re.DOTALL)
            changes_made += 1
        ...
        pattern = r"app\.post\s*\(\s*['\"]/api/chat/qwen/reload['\"]][^}]*}\s*\);?"
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: POST /api/chat/qwen/reload - Contiene '/reload'", content, flags=re.DOTALL)
            changes_made += 1
            print(f"✅ Eliminado: POST /api/chat/qwen/reload")
        ...
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: POST /api/chat/qwen/cached - Contiene '/qwen'", content, flags=re.DOTALL)
            changes_made += 1
        ...
        pattern = r"app\.post\s*\(\s*['\"]/api/chat/qwen/cached['\"]][^}]*}\s*\);?"
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: POST /api/chat/qwen/cached - Contiene '/cached'", content, flags=re.DOTALL)
            changes_made += 1
            print(f"✅ Eliminado: POST /api/chat/qwen/cached")
        ...
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, f"// ELIMINADO: GET /api/chat/qwen/status - Contiene '/qwen'", content, flags=re.DOTALL)
            changes_made += 1
