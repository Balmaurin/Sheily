...
from .module_validator import ModuleValidator, ModuleHealthStatus
...
class ModuleInitializationConfig:
    ...
    def __init__(
        self, 
        config_path: str = 'shaili_ai/config/config/module_initialization.json'
    ):
        """
        ...
        def missing_method_recovery(module, context):
            ...
            if "método requerido" in context['error']:
                ...
                def default_process(data):
                    ...
                    self.logger.warning(f"Usando método de procesamiento predeterminado para {module.__class__.__name__}")
                    return data
                    ...
                    try:
                        ...
                        for name, obj in module.__dict__.items():
                            if (
                                ...
                                not name.startswith('Token') and  # Excluir clases de tokens
                                not hasattr(obj, '__dataclass_fields__') and  # Excluir dataclasses
                                not name.endswith('Config') and  # Excluir clases de configuración
                                not name.endswith('Wallet') and  # Excluir clases de wallet
                                not name.endswith('Transaction')  # Excluir clases de transacción
                            ):
        ...
        for module_name in modules_to_initialize:
            ...
            try:
                ...
                if not self.config_manager.validate_module_dependencies(
                    module_name, 
                    list(self.initialized_modules.keys())
                ):
                ...
                module_health = self.module_validator.generate_module_report(
                    module_instance, 
                    available_modules=self.initialized_modules
                )
                ...
                if not module_health['is_healthy']:
                    recovered_module = self.module_validator.recover_module(
                        module_instance, 
                        ModuleHealthStatus(module_name)
                    )
