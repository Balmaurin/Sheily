...
def process_query(request: QueryRequest):
    ...
    try:
        ...
        tokens_used = len(request.query.split()) + len(
            llm_data.get("response", "").split()
        )
        ...
        if any(
            word in query_lower
            for word in ["python", "código", "programar", "desarrollo"]
        ):
            domain = "programming"
        elif any(
            word in query_lower
            for word in ["ia", "inteligencia", "modelo", "aprendizaje"]
        ):
            domain = "ai"
        ...
        logger.info(
            f"✅ Consulta procesada - Dominio: {domain}, Tiempo: {response_time:.2f}s, Tokens: {tokens_used}"
        )

        return QueryResponse(
            query=request.query,
            response=llm_data.get(
        ...
        raise HTTPException(
            status_code=504,
            detail="Timeout: El servidor LLM tardó demasiado en responder",
        )
    except requests.exceptions.ConnectionError:
        logger.error("Error de conexión con LLM server")
        raise HTTPException(
            status_code=503, detail="No se pudo conectar con el servidor LLM"
        )
...
if __name__ == "__main__":
    ...
    config = uvicorn.Config(
        app, host="0.0.0.0", port=8080, log_level="info", access_log=True
    )
