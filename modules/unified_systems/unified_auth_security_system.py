...
class UnifiedAuthSecuritySystem:
    ...
    def generate_jwt_token(self, user_id: str, claims: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            self._log_security_event(
                user_id, "jwt_token_generated", SecurityLevel.LOW,
    ...
    def revoke_jwt_token(self, token: str) -> bool:
        ...
        try:
            ...
            if token_id:
                ...
                self._log_security_event(
                    payload['user_id'], "jwt_token_revoked", SecurityLevel.MEDIUM,
    ...
    def setup_2fa(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log_security_event(
                user_id, "2fa_setup", SecurityLevel.HIGH,
                details={"method": "totp"}
            )
    ...
    def generate_recovery_token(self, user_id: str, ip_address: Optional[str] = None) -> str:
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                str(uuid.uuid4()), user_id, token_hash, ip_address,
            ...
            self._log_security_event(
                user_id, "recovery_token_generated", SecurityLevel.HIGH,
    ...
    def validate_recovery_token(self, user_id: str, recovery_token: str) -> bool:
        ...
        try:
            ...
            self._log_security_event(
                user_id, "recovery_token_used", SecurityLevel.HIGH
            )
    ...
    def _is_token_revoked(self, token: str) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT is_revoked FROM jwt_tokens WHERE token_id = ?",
                (token_id,)
            )
            result = cursor.fetchone()
            conn.close()
    ...
    def _log_security_event(self, user_id: str, event_type: str, severity: SecurityLevel,
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                event_id, user_id, event_type, severity.value, datetime.utcnow(),
                ip_address, json.dumps(device_info) if device_info else None,
                json.dumps(details) if details else None, risk_score
            ))
            conn.commit()
            conn.close()
...
class IntrusionDetectionSystem:
    ...
    def analyze_activity(self, user_id: str, activity_type: str,
                        ip_address: Optional[str] = None,
                        device_info: Optional[Dict[str, Any]] = None) -> float:
...
def get_unified_auth_security_system(db_path: Optional[str] = None) -> UnifiedAuthSecuritySystem:
