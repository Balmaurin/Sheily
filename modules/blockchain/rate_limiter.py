...
class RateLimiter:
    ...
    def __init__(self, config_path: str = "shaili_ai/config/rate_limits.json"):
        ...
        try:
            with self.lock:
                ...
                if current_count >= rule.config.max_requests:
                    ...
                    return False, f"Rate limit excedido. Cooldown hasta {cooldown_until}"
                ...
                if current_count >= rule.config.burst_limit:
                    ...
                    recent_requests = sum(1 for req_time in requests 
                                        if current_time - req_time < 60)  # últimos 60 segundos
    ...
    def record_request(self, user_id: str, rule_id: str) -> bool:
        ...
        try:
            with self.lock:
                ...
                max_history = self.rules.get(rule_id, RateLimitRule("", "", RateLimitConfig(100, 3600))).config.max_requests * 2
                requests = self.user_requests[user_id][rule_id]
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Usuarios en cooldown
            users_in_cooldown = sum(1 for user_cooldowns in self.cooldown_users.values() 
                                  if user_cooldowns)
