...
class DomainExpansionEngine:
    def __init__(
        ...
        if not self.base_path.exists():
            ...
            self.base_path.write_text(json.dumps(DEFAULT_SEED, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    def expand_with_samples(self, samples: List[str]) -> Dict:
        ...
        self.out_path.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
        return result
    ...
    def expand_domain_taxonomy(self, interactions):
        ...
        """
        try:
            self.logger.info(f"Expandiendo taxonomía de dominios con {len(interactions)} interacciones")
