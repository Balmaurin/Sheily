...
class RAGRetriever:
    ...
    def _get_document_by_id(self, session, idx: int, domain: Optional[str] = None) -> Optional[Dict]:
        """
            ...
            for dist, idx in zip(distances[0], indices[0]):
                ...
                if doc:
                    retrieved_docs.append(
                        self._build_retrieved_doc(doc, dist)
                    )
    ...
    def generate_rag_response(
        ...
        # Recuperar documentos
        retrieved_docs = self._search_documents(
            query_embedding, 
            domain=domain
        )
