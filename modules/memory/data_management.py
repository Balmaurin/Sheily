...
class DataManagementService:
    ...
    def export_user_data(
        ...
        try:
            ...
            user_data = session.query(UserData).filter(UserData.user_id == user_id).all()
    ...
    def cleanup_expired_data(self):
        ...
        try:
            ...
            deleted_count = session.query(UserData).filter(
                UserData.timestamp < expiration_date
            ).delete(synchronize_session=False)
            
            session.commit()
            
            self.logger.info(f"Limpieza de datos: {deleted_count} registros expirados eliminados")
            return deleted_count
        except Exception as e:
            session.rollback()
