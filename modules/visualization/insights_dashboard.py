...
class InsightsDashboard:
    ...
    def _setup_dashboard_callbacks(self):
        ...
        def update_semantic_clustering(n):
            ...
            insights = self.integration_manager.generate_advanced_insights(recent_interactions)
            
            # Convertir clusters a DataFrame
            clusters_data = []
            for cluster, texts in insights['semantic_clusters'].items():
                clusters_data.extend([
                    {"Cluster": cluster, "Texto": texto} 
                    for texto in texts
                ])
    ...
    def generate_report(
        ...
        # Obtener insights avanzados
        recent_interactions = self.integration_manager.continuous_improvement.get_recent_interactions()
        advanced_insights = self.integration_manager.generate_advanced_insights(recent_interactions)
