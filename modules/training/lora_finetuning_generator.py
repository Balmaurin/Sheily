...
class LoRAFinetuningGenerator:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_lora_branch_name ON lora_training_data (branch_name)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_lora_exercise_id ON lora_training_data (exercise_id)")
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_lora_user_id ON lora_training_data (user_id)")
    ...
    def _create_default_branch_specialists(self):
        ...
        default_specialists = [
            {
                ...
                "domain": "AI/ML",
                "performance_metrics": {"accuracy": 0.0, "response_quality": 0.0, "knowledge_depth": 0.0}
            },
            {
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.info(f"✅ {len(default_specialists)} especialistas de rama creados")
    ...
    def generate_lora_training_data(self, session_data: Dict[str, Any], exercise_data: Dict[str, Any]) -> List[LoRATrainingData]:
        ...
        try:
                ...
                if user_answer_data:
                    ...
                    training_data = LoRATrainingData(
                        ...
                        metadata={
                            ...
                            "complexity": question_data["metadata"].get("complexity", ""),
            ...
            logger.info(f"✅ Generados {len(training_data_list)} datos de entrenamiento para {branch_name}")
    ...
    def _determine_training_format(self, question_data: Dict[str, Any]) -> str:
        ...
        if any(word in question_text for word in ["¿cuál es", "qué es", "explica", "describe"]):
            return "open_ended"
    ...
    def _calculate_quality_score(self, user_answer: str, correct_answer: str, is_correct: bool, difficulty: str) -> float:
        ...
        try:
            ...
            if user_length > 0 and correct_length > 0:
                length_ratio = min(user_length / correct_length, correct_length / user_length)
            ...
            quality_score = base_score * difficulty_multiplier * length_factor * similarity_factor
    ...
    def _save_training_data(self, training_data_list: List[LoRATrainingData]):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.info(f"✅ {len(training_data_list)} datos de entrenamiento guardados")
    ...
    def get_training_data_for_branch(self, branch_name: str, limit: int = 100) -> List[LoRATrainingData]:
    ...
    def export_lora_dataset(self, branch_name: str, output_path: str = None) -> str:
        ...
        try:
            ...
            if output_path is None:
                output_path = f"shaili_ai/data/lora_datasets/{branch_name}_dataset.jsonl"
            ...
            logger.info(f"✅ Dataset exportado: {output_path} ({len(lora_dataset)} ejemplos)")
            return output_path
