...
def select_representative_branches(input_file='shaili-ai/branches/base_branches.json', num_branches=32):
    """
    ...
    for domain in domains:
        ...
        selected_domain_branches = domain_branches[:min(domain_branch_count, len(domain_branches))]
    ...
    while current_total < num_branches:
        for domain in domains:
            if current_total < num_branches:
                remaining_branches = set(original_branches[domain]) - set(selected_branches[domain])
