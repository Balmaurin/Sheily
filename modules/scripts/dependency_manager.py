...
class DependencyManager:
    ...
    def _register_system_modules(self):
        ...
        all_modules = (
            base_modules + embedding_modules + ai_modules + branch_modules +
            learning_modules + memory_modules + expert_modules + security_modules +
            blockchain_modules + evaluation_modules + search_modules + response_modules +
            data_modules + utility_modules + analysis_modules + emotion_modules +
            improvement_modules + component_modules + output_modules + model_modules +
            monitoring_modules + validation_modules + fusion_modules + demo_modules +
            type_modules
        )
        ...
        for module_name, dependencies in self.dependency_graph.items():
            for dep in dependencies:
                if dep not in self.modules:
                    logger.warning(f"⚠️ Dependencia no encontrada: {dep} para {module_name}")
