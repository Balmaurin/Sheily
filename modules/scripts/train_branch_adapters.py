...
class BranchEmbeddingTrainer:
    ...
    def prepare_training_data(self, branch_name: str, texts: List[str]) -> List[InputExample]:
        """
        ...
        """
        try:
            logger.info(f"📝 Preparando datos de entrenamiento para rama '{branch_name}'")
            ...
            for i, text in enumerate(texts):
                ...
                if i < len(texts) - 1:
                    ...
                    if i > 0:
                        examples.append(InputExample(texts=[text, texts[i - 1]], label=0.5))
    ...
    def train_branch_embeddings(
        ...
        try:
            ...
            train_dataloader = branch_model.get_dataloader(training_examples, batch_size=batch_size)
            ...
            self._save_branch_config(branch_name, branch_model, final_loss, evaluation_score)
            
            # Guardar en base de datos
            self._save_training_session(branch_name, len(training_texts), epochs, batch_size, learning_rate, final_loss, evaluation_score)
    ...
    def _evaluate_branch_model(self, model: SentenceTransformer, texts: List[str]) -> float:
        """
    ...
    def _save_training_session(
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (branch_name, data_count, epochs, batch_size, learning_rate, final_loss, evaluation_score))
