...
class ShailiRewardsIntegration:
    """
    Clase que integra el sistema de tracking, recompensas y precisión contextual
    """
    def __init__(self, 
                 sessions_path='rewards/sessions', 
                 vault_path='rewards/vault'):
        """
    ...
    def get_domain_performance(self, domain: str) -> dict:
        ...
        avg_quality = sum(
            session.get('quality_score', 0) for session in useful_sessions
        ) / len(useful_sessions) if useful_sessions else 0
