...
class ComponentTransformer:
    ...
    def migrate_component(
        ...
        try:
            ...
            if compatibility_report.compatibility_level == CompatibilityLevel.FULLY_COMPATIBLE:
                logger.info(f"Componente {component.__class__.__name__} ya es compatible")
                return component
            
            if compatibility_report.compatibility_level == CompatibilityLevel.REQUIRES_ADAPTATION:
            ...
            if compatibility_report.compatibility_level == CompatibilityLevel.INCOMPATIBLE:
                raise ValueError(f"Componente incompatible: {compatibility_report.required_adaptations}")
    ...
    def transform_module(self, module_path: str) -> str:
        ...
        try:
            ...
            # Formatear con Black
            try:
                transformed_code = black.format_str(transformed_code, mode=black.FileMode())
            except Exception as e:
...
def main():
    ...
    return {
        "status": "ok", 
        "message": "Toolkit de migración inicializado"
    }
