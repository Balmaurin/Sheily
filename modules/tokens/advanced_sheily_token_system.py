...
class SheilyTokenConfig:
    ...
    token_types: Dict[str, float] = field(default_factory=lambda: {
        "TRAINING": 1.0,
        "RESPONSE": 1.5,
        "INNOVATION": 2.0,
...
class AdvancedSheilyTokenSystem:
    ...
    async def generate_tokens_for_training_session(
        ...
        if quality_score < self.config.min_quality_threshold:
            self.logger.warning(f"Calidad insuficiente para generar tokens: {quality_score}")
            return []
    ...
    async def purchase_tokens(
        ...
        self.user_token_balances[buyer_id] = self.user_token_balances.get(buyer_id, 0) + len(tokens_to_transfer)
    ...
    def get_token_marketplace_listings(self) -> List[Dict[str, Any]]:
        ...
        """
        return [
            listing for listing in self.token_marketplace.values() 
            if listing['status'] == 'active'
        ]
