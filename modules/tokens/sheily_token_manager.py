...
from modules.tokens.advanced_sheily_token_system import AdvancedSheilyTokenSystem, SheilyToken, SheilyTokenConfig
    ...
    def get_top_contributors(
        ...
        for contrib in contributions:
            for token_id in contrib.get('tokens', []):
                ...
                if token:
                    user_scores[token.user_id] = user_scores.get(token.user_id, 0) + token.amount
...
async def main():
    ...
    for contrib in user_contributions:
        print(f"- Tipo: {contrib['type']}, Dominio: {contrib.get('domain', 'N/A')}, Tokens: {len(contrib['tokens'])}")
