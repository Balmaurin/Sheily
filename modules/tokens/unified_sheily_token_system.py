...
class SheilyTokenConfig:
    ...
    token_types: Dict[TokenType, float] = field(
...
class UnifiedSheilyTokenSystem:
    ...
    async def _validate_and_sign_token(self, token: SheilyToken):
        ...
        try:
            ...
            token.expires_at = datetime.now() + timedelta(
                days=self.config.token_expiration_days
            )

            # Calcular poder de voto
            token.voting_power = token.quality_score * self.config.token_types.get(
    ...
    async def create_governance_proposal(
        self, proposer_id: str, proposal_type: str, proposal_details: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def vote_on_proposal(
        self, user_id: str, proposal_id: str, vote: str
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_user_voting_power(self, user_id: str) -> float:
        ...
        user_tokens = [
            ...
            and token.validation_status == TokenStatus.ACTIVE
        ]
    ...
    async def main():
        ...
        voting_result = await token_system.vote_on_proposal(
            user_id="user123", proposal_id=governance_proposal["id"], vote="yes"
        )
