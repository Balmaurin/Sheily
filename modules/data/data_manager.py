...
class DataManager:
    ...
    def __init__(self, data_dir: str = "data", db_url: str = "postgresql://user:password@localhost/sheily_db"):
    ...
    def store_user_interaction(self, user_id: str, session_id: str, interaction_type: str,
                             content: str, response: str, duration_ms: int = 0,
    ...
    def store_learning_data(self, branch_name: str, question: str, answer: str,
                          difficulty: float = 0.5, category: str = "general") -> str:
    ...
    def get_learning_data(self, branch_name: str = None, category: str = None, limit: int = 100) -> List[Dict[str, Any]]:
    ...
    def store_rag_memory(self, content: str, embedding: np.ndarray, source: str = "unknown",
                        domain: str = "general", relevance_score: float = 1.0,
                        metadata: Dict[str, Any] = None) -> str:
    ...
    def search_rag_memory(self, query_embedding: np.ndarray, limit: int = 10) -> List[Dict[str, Any]]:
        """Buscar en memoria RAG"""
        try:
            session = self.Session()
            try:
                # Obtener todos los embeddings de memoria RAG
                query = session.query(RAGMemory).order_by(
                    RAGMemory.relevance_score.desc(), 
                    RAGMemory.last_accessed.desc()
                ).limit(limit)
        ...
        with self._lock:
            try:
                ...
                try:
                    ...
                    deleted_interactions = session.query(UserInteraction).filter(
                        UserInteraction.timestamp < cutoff_date
                    ).delete(synchronize_session=False)
                    
                    # Limpiar datos de aprendizaje antiguos
                    deleted_learning_data = session.query(LearningData).filter(
                        LearningData.created_at < cutoff_date
                    ).delete(synchronize_session=False)
                    
                    session.commit()
                    
                    self.logger.info(f"🧹 Datos antiguos eliminados: {deleted_interactions} interacciones, {deleted_learning_data} registros de aprendizaje")
    ...
    def _load_faiss_index(self):
        ...
        if index_path.exists():
            try:
                ...
                self.logger.info(f"✅ Índice FAISS cargado: {self.faiss_index.ntotal} vectores")
            except Exception as e:
    ...
    def save_faiss_index(self):
        ...
        if self.faiss_index is not None:
            try:
                ...
                self.logger.info(f"💾 Índice FAISS guardado: {self.faiss_index.ntotal} vectores")
            except Exception as e:
