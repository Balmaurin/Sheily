...
class DailyExerciseIntegrator:
    ...
    async def integrate_daily_exercises(self) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info("🔄 Integrando ejercicios diarios con sistema de entrenamiento...")
            ...
            if not daily_exercises:
                ...
                return {"status": "no_exercises", "message": "No hay ejercicios diarios para hoy"}
            ...
            if converted_exercises:
                ...
                return {"status": "no_conversions", "message": "No se pudieron convertir ejercicios"}
                
        except Exception as e:
            self.logger.error(f"❌ Error integrando ejercicios diarios: {e}")
            return {"status": "error", "message": str(e)}
    
    def _convert_to_training_exercise(self, daily_exercise: Exercise) -> Optional[TrainingExercise]:
        """
        ...
        try:
            ...
            self.logger.error(f"❌ Error convirtiendo ejercicio {daily_exercise.id}: {e}")
            return None
    
    def _create_questions_from_exercise(self, exercise: Exercise) -> List[Dict[str, Any]]:
        """
        Crear preguntas basadas en el contenido del ejercicio diario
        """
        questions = []
        ...
        questions.append(TrainingQuestion(
            ...
            metadata={"type": "comprehension", "options": ["básico", "intermedio", "avanzado"]}
        ))
        ...
        try:
            ...
            for branch, exercise in exercises.items():
                ...
                if not existing:
                    ...
                    self.logger.info(f"✅ Ejercicio {exercise.id} agregado al sistema de entrenamiento")
                else:
                    self.logger.info(f"ℹ️ Ejercicio {exercise.id} ya existe en el sistema")
    ...
    async def generate_and_integrate(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not daily_exercises:
                ...
                return {"status": "no_generation", "message": "No se generaron nuevos ejercicios"}
