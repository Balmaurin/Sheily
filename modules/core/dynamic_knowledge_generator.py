...
class DynamicKnowledgeGenerator:
    ...
    def __init__(self, knowledge_db_path: Optional[str] = None):
        ...
        self.domain_patterns = {
            "programming": {
                "keywords": ["código", "python", "javascript", "función", "variable", "algoritmo"],
                "context_template": "En el contexto de programación, considerando las mejores prácticas..."
            },
            "ai": {
                "keywords": ["inteligencia artificial", "machine learning", "neural network", "modelo", "entrenamiento"],
    ...
    def generate_contextual_knowledge(self, context: str, domain: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            contextual_knowledge = {
                ...
                "confidence": self._calculate_confidence(db_knowledge, dynamic_knowledge),
    ...
    def _get_relevant_knowledge(self, context: str, domain: str) -> List[Dict[str, Any]]:
    ...
    def _generate_dynamic_context(self, context: str, domain: str) -> Dict[str, Any]:
        ...
        try:
            domain_pattern = self.domain_patterns.get(domain, self.domain_patterns["general"])
            
            dynamic_context = {
                "context_template": domain_pattern["context_template"],
                "relevant_keywords": [kw for kw in domain_pattern["keywords"] if kw in context.lower()],
    ...
    def _calculate_confidence(self, db_knowledge: List[Dict], dynamic_knowledge: Dict) -> float:
        """Calcular confianza del conocimiento generado"""
        try:
            # Confianza basada en conocimiento de BD
            db_confidence = 0.0
            if db_knowledge:
                db_confidence = sum(item.get("confidence", 0.0) for item in db_knowledge) / len(db_knowledge)
    ...
    def update_knowledge_base(self, topic: str, content: str, category: str, confidence: float = 0.8):
