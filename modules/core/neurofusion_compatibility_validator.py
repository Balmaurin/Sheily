    ...
    def validate_component(
        ...
        try:
            ...
            if "mse" in metrics_config["metrics"]:
                results["metrics"]["mse"] = mean_squared_error(ground_truth, predictions)
            ...
            if "precision" in metrics_config["metrics"]:
                results["metrics"]["precision"] = precision_score(ground_truth, predictions, average='weighted')
            
            if "recall" in metrics_config["metrics"]:
                results["metrics"]["recall"] = recall_score(ground_truth, predictions, average='weighted')
            
            if "f1" in metrics_config["metrics"]:
                results["metrics"]["f1"] = f1_score(ground_truth, predictions, average='weighted')
            
            # Evaluar estado de validación
            avg_performance = np.mean(list(results["metrics"].values()))
            results["validation_status"] = (
                "passed" if avg_performance >= metrics_config["performance_threshold"]
                else "failed"
            )
            ...
            if results["validation_status"] == "failed":
                ...
                logger.warning(f"Componente {component.config.name} no pasó validación experimental")
...
def main():
    ...
    return {
        "status": "ok", 
        "message": "Sistema de Compatibilidad inicializado"
    }
