...
class MemoryAnalytics:
    ...
    def analyze_conversation_patterns(self, user_id: Optional[str] = None, days: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            with sqlite3.connect(self.short_term_memory.config.database_path) as conn:
                ...
                if results:
                    first_message = datetime.fromtimestamp(min(row[3] for row in results))
                    last_message = datetime.fromtimestamp(max(row[4] for row in results))
    ...
    def find_semantic_clusters(self, top_k: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.semantic_memory.config.database_path) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT concept, description, embedding FROM semantic_memory")
    ...
    def analyze_episodic_memory(self, 
