...
def kill_existing_services():
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        try:
            ...
            if any(
                ...
                logger.info(
                    f"🔄 Terminando proceso: {proc.info['name']} (PID: {proc.info['pid']})"
                )
                proc.terminate()
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline"]):
        try:
            ...
            if any(
                pattern in cmdline
                for pattern in ["gateway_maestro_unificado.py", "run_llama_chat.py"]
            ):
                logger.warning(
                    f"🔨 Forzando terminación: {proc.info['name']} (PID: {proc.info['pid']})"
                )
                proc.kill()
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            continue
