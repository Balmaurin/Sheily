...
class GatewayMaestroUnificado:
    ...
    async def start_service(self, service_name: str) -> bool:
        ...
        for dep in config.dependencies:
            ...
            if not dep_status or dep_status.status != "running":
                logger.warning(f"⚠️ Dependencia {dep} no está ejecutándose para {service_name}")
                return False
        ...
        if service_name != "postgresql" and self._is_port_in_use(config.port):
            ...
            try:
                ...
                logger.info(f"🔄 Puerto {config.port} ocupado, usando {new_port} para {service_name}")
                config.port = new_port
                # Actualizar variables de entorno si es necesario
                if "REACT_APP_API_URL" in config.env_vars:
                    config.env_vars["REACT_APP_API_URL"] = f"http://localhost:{new_port}"
            except RuntimeError as e:
                logger.error(f"❌ No se pudo encontrar puerto libre para {service_name}: {e}")
                return False
        ...
        try:
            ...
            if status.status == "running":
                logger.info(f"✅ {config.name} iniciado exitosamente (PID: {process.pid}, Puerto: {config.port})")
                return True
            else:
        ...
        while time.time() - start_time < timeout:
            ...
            if service_name in self.processes:
                ...
                if process.poll() is not None:
                    # El proceso terminó
                    status.status = "failed"
                    status.error_message = f"Proceso terminó con código {process.returncode}"
                    return
        ...
        try:
            ...
            if status.health == "unhealthy" and config.restart_on_failure:
                if status.restart_count < config.max_restarts:
                    logger.warning(f"⚠️ Reiniciando {config.name} (intento {status.restart_count + 1})")
                    await self.stop_service(service_name)
                    status.restart_count += 1
                    await asyncio.sleep(5)
                    await self.start_service(service_name)
                else:
                    logger.error(f"❌ {config.name} falló demasiadas veces, no reiniciando más")
    ...
    def print_status(self):
        ...
        if "frontend" in self.service_status and self.service_status["frontend"].status == "running":
            print(f"   🎨 Frontend: http://localhost:{self.service_status['frontend'].port}")
        if "backend" in self.service_status and self.service_status["backend"].status == "running":
            print(f"   ⚙️ Backend API: http://localhost:{self.service_status['backend'].port}")
        if "llm_server" in self.service_status and self.service_status["llm_server"].status == "running":
            print(f"   🧠 LLM Server: http://localhost:{self.service_status['llm_server'].port}")
        if "ai_system" in self.service_status and self.service_status["ai_system"].status == "running":
            print(f"   🤖 AI System: http://localhost:{self.service_status['ai_system'].port}")
    ...
    async def stop_all_services(self):
        ...
        stop_order = list(reversed(["frontend", "ai_system", "blockchain", "backend", "llm_server"]))
