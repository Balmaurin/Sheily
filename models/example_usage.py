    ...
    try:
        ...
        if stats['top_operations']:
            ...
            for op in stats['top_operations'][:3]:
                print(f"   - {op['operation']}: {op['avg_time']:.2f}s ({op['count']} veces)")
...
def example_model_validation():
    ...
    try:
        ...
        print(f"   - Tiene configuración: {'✅' if path_validation['has_config'] else '❌'}")
        print(f"   - Tiene archivos de modelo: {'✅' if path_validation['has_model_files'] else '❌'}")
        
        # Detalles de compatibilidad
        compatibility = validation['compatibility']
        print(f"   - Dispositivo soportado: {'✅' if compatibility['device_supported'] else '❌'}")
        print(f"   - Memoria suficiente: {'✅' if compatibility['memory_sufficient'] else '❌'}")
        print(f"   - Dependencias cumplidas: {'✅' if compatibility['dependencies_met'] else '❌'}")
