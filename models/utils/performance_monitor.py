...
class PerformanceMonitor:
    ...
    def record_metric(self, operation: str, duration: float, metadata: Optional[Dict[str, Any]] = None, success: bool = True):
        """
        ...
        with self._lock:
            ...
            if duration > self.critical_threshold:
                self.logger.critical(f"Operación crítica lenta: {operation} tomó {duration:.2f}s")
            elif duration > self.slow_threshold:
                self.logger.warning(f"Operación lenta: {operation} tomó {duration:.2f}s")
    ...
    def end_operation(self, operation_id: str, metadata: Optional[Dict[str, Any]] = None, success: bool = True):
        """
        ...
        for operation, metrics in self.metrics.items():
            ...
            if slow_metrics:
                slow_ops.append({
                    ...
                    "avg_duration": statistics.mean([m.duration for m in slow_metrics]),
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        if all_times:
            ...
            n = len(sorted_times)
            if n % 2 == 0:
                summary["median_time"] = (sorted_times[n//2 - 1] + sorted_times[n//2]) / 2
            else:
    ...
    def set_thresholds(self, slow: float = 5.0, critical: float = 30.0):
        ...
        if slow > 0 and critical > slow:
            ...
            self.logger.info(f"Umbrales actualizados: slow={slow}s, critical={critical}s")
        else:
