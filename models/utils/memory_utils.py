...
class MemoryManager:
    ...
    def get_system_memory_stats(self) -> MemoryStats:
        ...
        try:
            ...
            self.logger.error(f"Error obteniendo estadísticas de memoria del sistema: {e}")
    ...
    def get_device_memory_stats(self, device: str) -> MemoryStats:
        ...
        try:
            ...
            self.logger.error(f"Error obteniendo estadísticas de memoria de {device}: {e}")
    ...
    def get_memory_trends(self, device: str = "system", window: int = 10) -> Dict[str, Any]:
        """
        ...
        # Calcular tendencia
        recent = device_history[-window:] if len(device_history) >= window else device_history
        old_avg = sum(s.percentage_used for s in recent[:len(recent)//2]) / (len(recent)//2)
        new_avg = sum(s.percentage_used for s in recent[len(recent)//2:]) / (len(recent)//2)
    ...
    def estimate_model_memory_usage(self, model_size_gb: float, batch_size: int = 1) -> Dict[str, float]:
        """
    ...
    def can_load_model(self, model_size_gb: float, device: str = "auto") -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "recommended_batch_size": self.device_utils.get_recommended_batch_size(device, model_size_gb)
        }
    ...
    def set_memory_thresholds(self, warning: float = 0.8, critical: float = 0.95):
        ...
        if 0 <= warning <= 1 and 0 <= critical <= 1 and warning < critical:
            ...
            self.logger.info(f"Umbrales de memoria actualizados: warning={warning}, critical={critical}")
        else:
            raise ValueError("Umbrales inválidos: warning < critical y ambos entre 0 y 1")
