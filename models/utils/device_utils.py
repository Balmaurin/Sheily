...
class DeviceUtils:
    ...
    def get_optimal_device(self, device_preference: str = "auto") -> str:
        ...
        if device_preference == "auto":
            ...
            self.logger.warning(f"Dispositivo {device_preference} no disponible, usando CPU")
            return "cpu"
    ...
    def get_device_info(self, device: str) -> Optional[DeviceInfo]:
        ...
        if device in self.devices:
            ...
            if device.startswith("cuda:"):
                ...
                device_info.memory_used = torch.cuda.memory_allocated(device_id) / (1024**3)
                device_info.memory_available = device_info.memory_total - device_info.memory_used
    ...
    def get_best_device_for_model(self, model_size_gb: float) -> str:
        ...
        for device_name, device_info in self.devices.items():
            if device_info.is_available and device_info.memory_available >= model_size_gb * 1.5:
                return device_name
        ...
        self.logger.warning(f"No hay dispositivo con suficiente memoria para modelo de {model_size_gb}GB")
        return best_device
    ...
    def set_device_memory_fraction(self, device: str, fraction: float) -> bool:
        ...
        try:
            if device.startswith("cuda:"):
                ...
                self.logger.info(f"Fracción de memoria establecida en {fraction} para {device}")
                return True
            else:
                self.logger.warning(f"No se puede establecer fracción de memoria para {device}")
                return False
        except Exception as e:
