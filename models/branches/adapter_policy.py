...
class AdapterUpdatePolicy:
    ...
    def _update_performance_metrics(self, domain: str, metrics: Dict[str, Any]):
        ...
        self.performance_metrics[domain].update({
            ...
            'usage_count': self.performance_metrics[domain].get('usage_count', 0) + 1
        })
    ...
    def _should_update_adapter(self, domain: str) -> bool:
        ...
        if datetime.now() - last_updated > timedelta(seconds=self.config['update_frequency']):
            return True
    ...
    def _compress_inactive_adapters(self):
        ...
        try:
            ...
            for domain, adapter_info in self.adapter_cache.items():
                last_used = datetime.fromisoformat(adapter_info.get('last_used', '2000-01-01'))
    ...
    def _get_cache_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_usage": self._get_memory_usage(),
            "compressed_adapters": sum(1 for info in self.adapter_cache.values() if info.get('compressed', False)),
            "cache_hits": sum(info.get('cache_hits', 0) for info in self.adapter_cache.values()),
            "cache_misses": sum(info.get('cache_misses', 0) for info in self.adapter_cache.values())
        }
