...
class BranchManager:
    ...
    def __init__(self, base_model_path: str = "models/custom/shaili-personal-model"):
        ...
        self.logger.info(f"✅ BranchManager inicializado con {len(self.domain_to_branch)} dominios")
    ...
    def _load_branches_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"Configuración de ramas no encontrada en {config_path}")
            return {"domains": []}
    ...
    def _load_micro_branches(self) -> Dict[str, List[str]]:
        ...
        for domain, branch_path in self.domain_to_branch.items():
            ...
            if os.path.exists(full_path):
                try:
                    with open(f"{full_path}/microramas.json", 'r', encoding='utf-8') as f:
                        data = json.load(f)
