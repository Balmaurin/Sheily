...
from models import (
    get_model_manager, create_model, load_model, get_model,
    generate_text, get_embeddings, list_models, get_system_stats,
    cleanup, ModelConfig, CausalLMModel, EmbeddingModel
)
...
def test_model_operations():
    ...
    if model:
        ...
        print(f"✅ Métricas actualizadas: {model.metrics.requests_processed} requests, {model.metrics.tokens_generated} tokens")
...
def test_performance():
    ...
    end_time = time.time()
    print(f"✅ Tiempo de estadísticas del registro: {(end_time - start_time)*1000:.2f}ms")
