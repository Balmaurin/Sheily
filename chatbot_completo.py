...
class SHEILYCompleteChatbot:
    ...
    def _initialize_memory_systems(self):
        ...
        try:
            ...
            self.short_term_memory = ShortTermMemory(
                max_messages=20, max_tokens=2048, summary_every=4000
            )
    ...
    def _initialize_learning_system(self):
        ...
        try:
            from modules.learning.neural_plasticity_manager import (
    ...
    def _load_specialized_prompts(self):
        ...
        try:
                ...
                for branch_file in branches_dir.glob("*.json"):
                    try:
                        with open(branch_file, "r", encoding="utf-8") as f:
                            ...
                            self.branch_prompts[branch_name] = branch_data.get(
                                "system_prompt", ""
                            )
                    except Exception as e:
            ...
            logger.info(
                f"✅ Cargados {len(self.branch_prompts)} prompts especializados"
            )
        ...
        try:
            ...
            if self.branch_manager:
                try:
                    ...
                    for branch in branches:
                        ...
                        if self.context.memory_context:
                            memory_summary = self.context.memory_context.get(
                                "summary", ""
                            ).lower()
                            for keyword in branch.keywords:
            ...
            if self.advanced_training:
                try:
                    ...
                    if any(
                        word in query.lower()
                        for word in ["explica", "enseña", "cómo", "qué es", "por qué"]
                    ):
    ...
    def generate_hybrid_response(self, query: str, analysis: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if response_approach == "branch_specialized":
                response = self._generate_branch_response(
                    query, analysis, system_prompt, hybrid_context
                )
                ...
                response = self._generate_memory_response(
                    query, analysis, system_prompt, hybrid_context
                )
            else:
        ...
        if analysis.get("branch_recommendation"):
            ...
            if branch.keywords:
                context_parts.append(
                    f"Palabras clave: {', '.join(branch.keywords[:5])}"
                )
    ...
    def _generate_branch_response(
        self, query: str, analysis: Dict[str, Any], system_prompt: str, context: str
    ) -> str:
        ...
        try:
            if self.lora_trainer:
                training_data = {
                    ...
                    "response": response,
                    "domain": analysis.get("domain", "general"),
                    "branch": analysis.get("branch_recommendation", {}).get(
                        "name", "general"
                    ),
    ...
    def get_complete_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "specialized_systems": {
                ...
                "available_branches": (
                    list(self.branch_prompts.keys()) if self.branch_prompts else []
                ),
                "available_lora": (
                    list(self.lora_models.keys()) if self.lora_models else []
                ),
            },
        }
...
def start_complete_chatbot():
    ...
    print(
        f"🎯 Ramas especializadas: {status['specialized_systems']['branch_prompts_loaded']}"
    )
    ...
    while True:
        try:
            ...
            if user_input.lower() == "estado":
                ...
                print(
                    f"  • Conversación: {status['context']['conversation_length']} mensajes"
                )
                print(f"  • Dominio actual: {status['context']['current_domain']}")
                print(
                    f"  • Rama activa: {status['context']['active_branch'] or 'Ninguna'}"
                )
                print(
                    f"  • Adaptadores LoRA: {len(status['context']['lora_adapters'])}"
                )
                print(
                    f"  • Componentes activos: {sum(status['components'].values())}/{len(status['components'])}"
                )
                continue
            ...
            if user_input.lower() == "memoria":
                if chatbot.short_term_memory:
                    ...
                    print(
                        f"  • Mensajes en memoria: {len(chatbot.short_term_memory.messages)}"
                    )
                    print(
                        f"  • Tokens actuales: {chatbot.short_term_memory.current_tokens}"
                    )
                    print(
                        f"  • Límite de mensajes: {chatbot.short_term_memory.max_messages}"
                    )
                else:
            ...
            if user_input.lower().startswith("analizar "):
                ...
                if text:
                    ...
                    if analysis.get("semantic_analysis"):
                        print(
                            f"  • Temas principales: {analysis['semantic_analysis'].get('main_topics', [])}"
                        )
                    if analysis.get("branch_recommendation"):
                        print(
                            f"  • Rama recomendada: {analysis['branch_recommendation'].name}"
                        )
            ...
            if result["analysis"].get("branch_recommendation"):
                print(
                    f"🎯 Rama especializada: {result['analysis']['branch_recommendation'].name}"
                )
