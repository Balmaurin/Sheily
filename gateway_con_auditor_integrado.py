...
class GatewayMaestroConAuditor:
    def __init__(self):
        ...
        self.service_configs = {
            ...
            "ai_system": {
                ...
                "start_cmd": [
                    "python3",
                    "/home/yo/sheily-ai/modules/unified_systems/simple_ai_server.py",
                ],
                "port": 8080,
    ...
    def audit_endpoints(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["python3", "/home/yo/sheily-ai/gateway_auditor_endpoints.py"],
                capture_output=True,
                text=True,
                timeout=120,
            )
            ...
            if result.returncode == 0:
                ...
                if efficiency_match and total_match and working_match:
                    ...
                    if efficiency < 85.0 and cleanable > 5:
                        self.logger.warning(
                            f"⚠️ Eficiencia baja ({efficiency}%) - {cleanable} endpoints limpiables"
                        )
                        self.logger.info(
                            f"💡 Sugerencia: Ejecutar 'python3 limpieza_endpoints_manual.py'"
                        )
    ...
    def start_service(self, service_name, config):
        ...
        if service_name == "postgresql":
            ...
            try:
                result = subprocess.run(
                    config["check_cmd"], capture_output=True, timeout=5
                )
                if result.returncode == 0:
        ...
        if not self.check_port_available(port):
            ...
            if new_port:
                self.logger.info(
                    f"🔄 Puerto {port} ocupado, usando {new_port} para {service_name}"
                )
                port = new_port
                config["port"] = new_port
                # Actualizar URL de salud si existe
                if config.get("health_url"):
                    config["health_url"] = config["health_url"].replace(
                        f":{config['port']}", f":{new_port}"
                    )
            else:
                self.logger.error(
                    f"❌ No se encontró puerto disponible para {service_name}"
                )
                return False
                ...
                if config.get("health_url"):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            self.logger.info(
                                f"✅ {config['name']} iniciado exitosamente (PID: {process.pid}, Puerto: {port})"
                            )
                            return True
                    except:
                        pass
    ...
    def start_all_services(self):
        ...
        if started == total:
            ...
            self.logger.warning(
                f"⚠️ Solo {started} de {total} servicios iniciaron correctamente"
            )
            return False
